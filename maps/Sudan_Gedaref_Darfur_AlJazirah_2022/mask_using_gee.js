...
for (var i in S2display) {
    Map.addLayer(
        image,
        { bands: [i + '_B4', i + '_B3', i + '_B2'], min: 1000, max: 6000 },
        'S2 ' + S2display[i]
    ).setShown(0);
}
...
var assetName =
    'projects/bsos-geog-harvest1/assets/harvest-reference-datasets/SudanGedarefDarfurAlJazirah2022';
var points = ee.FeatureCollection(assetName);

var trainingPts = points.filter(
    ee.Filter.or(
...
var worldCereal = ee
    .ImageCollection(
        ee
            .ImageCollection('ESA/WorldCereal/2021/MODELS/v100')
    ...
    .rename('crop');
Map.addLayer(
    worldCereal.selfMask(),
    classVis,
    'Cropland from WorldCereal'
).setShown(0);
...
var trainedRf = ee.Classifier.smileRandomForest({ numberOfTrees: 20 })
    ...
    .train({
        features: training,
        classProperty: 'class_probability',
...
var classVis = {
    min: 0,
    max: 1.0,
    palette: palettes.cmocean.Speed[7].slice(0, -2),
};
